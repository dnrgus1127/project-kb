...
지식 문서는 Obsidian 보관함에 `<vault>/me/<프로젝트>/` 형태로 저장되며, 영역 폴더(00 백로그·1~5·90 논의사항·91 코드리뷰·92 발견된 이슈) + 시각 자료(`images/`) + MOC(📑 인덱스) + 전 프로젝트 카탈로그로 구성된다.
...
새 프로젝트          /kb-init  →  영역 폴더(00·1~5·90·91·92) + images/ + 📑 인덱스 + 카탈로그 등록
...
└── <프로젝트>/
    ...
    ├── 92. 발견된 이슈/           # 다른 작업 중 발견한 미착수 결함·이슈 (작업성)
    └── images/                    # 흐름도·구성도 SVG 등 시각 자료
```

### 프로젝트별 구조 오버라이드

위 구조는 **전역 기본값**이다. 특정 프로젝트가 다른 폴더 구조/규칙이 필요하면 프로젝트 폴더 루트에 `conventions.md`를 둔다(전역과 같은 이름, 위치만 다름). 전역을 베이스로 **델타 오버라이드**되어 적은 섹션만 덮어쓰고 나머지는 전역을 따른다(`conventions.md` §12, §0·§10은 잠금). `kb-init`이 신규 프로젝트 생성 시 "커스텀 구조가 필요한가"를 물어 **온디맨드**로 만든다.
...
- **프로젝트별 오버라이드**: 프로젝트 폴더의 `conventions.md`로 전역 규칙을 델타 오버라이드(§12). 온디맨드 생성, §0·§10은 잠금.
